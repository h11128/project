...
for i in dicts:
    ...
    dicts[i][2] = smooth(dicts[i][2])
    dicts[i][3] = smooth(dicts[i][3])
    dicts[i][4] = smooth(dicts[i][4])
    dicts[i][5] = smooth(dicts[i][5])
    dicts[i][6] = smooth(dicts[i][6])

"""predict from test set, use laplace smooth and calculate whole probality"""
...
for k in range(len(x_test)):
    ...
    for i in dicts:
        if i in set(x_test[k]):
            ...
            """
            p1 += log10(dicts[i][2]) -log10(dicts[i][6])
        else:
            p1 += log10(dicts[i][3]) - log10(smooth(1-dicts[i][6]))
